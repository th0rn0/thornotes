...
func (r *APITokenRepo) Create(ctx context.Context, userID int64, name, token string) (*model.APIToken, error) {
	const q = `
		...
		VALUES (?, ?, ?)
		RETURNING id, user_id, name, token, created_at, last_used_at`

	row := r.writeDB.QueryRowContext(ctx, q, userID, name, token)
...
func (r *APITokenRepo) GetByToken(ctx context.Context, token string) (*model.APIToken, error) {
	...
	row := r.readDB.QueryRowContext(ctx, q, token)
	t, err := scanAPIToken(row)
	if err != nil {
...
func scanAPIToken(s scanner) (*model.APIToken, error) {
	...
	if err := s.Scan(&t.ID, &t.UserID, &t.Name, &t.Token, &t.CreatedAt, &t.LastUsedAt); err != nil {
		return nil, err
	}
	if len(t.Token) >= 8 {
		t.Prefix = t.Token[:8]
