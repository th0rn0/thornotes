...
func NewJournalRepo(readDB, writeDB *sql.DB) *JournalRepo {
	return &JournalRepo{readDB: readDB, writeDB: writeDB}
}
...
func (r *JournalRepo) Create(ctx context.Context, userID int64, name string) (*model.Journal, error) {
	...
	err := r.writeDB.QueryRowContext(ctx,
		`INSERT INTO journals (user_id, name) VALUES (?, ?) RETURNING id`,
		userID, name,
	).Scan(&id)
	if err != nil {
